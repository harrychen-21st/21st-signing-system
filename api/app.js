...
async function createApp() {
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    let userInfo = { name: "\u9810\u8A2D\u6E2C\u8A66\u54E1 (Test)", dept: "CS (\u5BA2\u670D\u8655)", manager: "", roles: "ROLE:EMPLOYEE" };
    if (scriptUrl) {
      try {
...
| **\u7B2C 7 \u95DC** | \u5C0F\u7AE0\u7BA1\u7406\u4EBA (ROLE:SMALL_SEAL_MGR) | **seal_type != '\u767C\u7968\u7AE0'** | \u5C0F\u7AE0\u5C08\u8CAC\u4FDD\u7BA1\u4EBA\u54E1\u57F7\u884C\u7528\u5370\u64CD\u4F5C\u8207\u767B\u8A18 |`,
  ...
  app.get("/api/tickets/pending/:email", authMiddleware, async (req, res) => {
    const email = req.params.email.toLowerCase();
    const scriptUrl = process.env.GOOGLE_APPS_SCRIPT_URL;
    const mockTickets = [
      { id: "DEMO-AP-001", createdAt: (/* @__PURE__ */ new Date()).toISOString(), applicantEmail: "test@company.com", applicantName: "\u9673\u5C0F\u660E (\u5C55\u793A\u7528)", dept: "MK (\u884C\u92B7\u4F01\u5283\u90E8)", formType: "AP", subject: "\u884C\u92B7\u5408\u4F5C\u5C08\u6848\u7C3D\u5448", amount: "", status: "Pending", stage: "1" }
    ];
    if (!scriptUrl) {
    ...
    try {
      ...
      const myRow = (usersData.data || []).find((r) => r[0]?.toLowerCase() === email);
      const myRolesStr = String(myRow && myRow[4] ? myRow[4] : "");
      const myRoles = myRolesStr ? myRolesStr.split(",").map((r) => r.trim()) : [];
      ...
      const pendingTickets = ticketsRows.slice(1).filter((row) => {
        const tStatus = row[6];
        const tApprover = row[13];
        ...
        const stageRule = allRules.find((r) => r[1] === tFormType && Number(r[2]) === tStage);
  ...
  app.post("/api/tickets/:ticketId/action", authMiddleware, async (req, res) => {
    const { ticketId } = req.params;
    const { action, approverEmail, comment, formDataUpdates } = req.body;
    const scriptUrl = process.env.GOOGLE_APPS_SCRIPT_URL;
    ...
    try {
      ...
      const allRules = rulesData.data || [];
      const currentRule = allRules.find((r) => r[1] === formType && Number(r[2]) === currentStage);
      ...
      const response = await fetch(scriptUrl, {
        ...
        body: JSON.stringify({
          ...
          actionType: action,
          comment,
          formData: formDataUpdates ? formData : void 0
