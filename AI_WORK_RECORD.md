...
### 已完成調整
- 依提供的 `APMK20260904002.pdf` 檢視結果，原輸出有金額未加千分位、AP 簽核欄位僅三欄，且簽核區溢到第 2 頁的情形。
- 系統中涉及金額的顯示改為千分位格式，例如 `50000` 顯示為 `50,000`；資料庫原始值不加逗號，避免影響後續計算。
- AP 簽呈單列印/PDF 的簽核欄位改為五欄：`總經理`、`管理本部長`、`單位本部長`、`單位處主管`、`申請人`。
- AP 列印版面縮小頁邊距與區塊間距，讓新增五欄簽核區後仍盡量維持一頁輸出。

---

## 2026-09-04 簽呈單 AP 欄位與列印/PDF 調整

### 已完成調整
- AP 新增 `related_case_no` 相關案件編號，供填寫內部系統案件號；此欄位不等同於 `related_ticket` 申請單號勾稽欄位。
- AP 新增 `estimated_amount` 預估金額，送出時同步寫入既有 `Tickets.Amount`，供後台與稽核匯出使用。
- AP 在 `external_collab = 是` 時新增 `applicant_related_party` 是否為關係人，僅作為申請人自評留痕，不取代 AML DB 查核結果。
- 送出涉及外部公司的 AP 時，Apps Script 會依統編查找 AML DB 既有紀錄，將既有 `風控AML` 與 `關係人(是/否)` 回傳前端並寫入 `Tickets` 的 AML/RP 欄位。
- AP 列印/PDF 會簽文字規則：
  - 若既有 `風控AML = 沒有找到任何紀錄，OK` 且 `關係人(是/否) = 否`，壓上「經管理處查核非屬關係人交易，且經第三方確認查無反社會或暴力團體相關負面新聞」。
  - 若既有 `風控AML = 沒有找到任何紀錄，OK` 且 `關係人(是/否)` 包含「已過關係人會議」，壓上「經管理處查核為關係人交易且已過關係人會議，且經第三方確認查無反社會或暴力團體相關負面新聞」。
  - 其他狀態、Pending、需判別風險或未查過時，顯示空白核取方塊加同一段關係人會議通過文字，供人工確認後勾選。
- 送出後列印與歷史申請列印都新增頁碼 `第 1 頁 / 共 1 頁`，並新增不指定職稱的簽核留白欄位，留白高度較原先加高。

### DB 影響
- 不新增 `Tickets` 表頭；`related_case_no` 與 `applicant_related_party` 存於 `FormData_JSON`。
- `estimated_amount` 同步寫入 `Tickets.Amount`。
