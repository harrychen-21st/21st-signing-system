...
export default function TrackDashboard({ user }: { user: any }) {
  ...
  const handleEditClick = (ticket: MyTicket) => {
    setEditTicket(ticket);
    setEditFormData(ticket.formData || {});
  };
  ...
  const handleResubmit = async () => {
    ...
    try {
      const res = await authFetch(`/api/tickets/${editTicket.id}/resubmit`, {
        method: 'POST',
      ...
      if (data.success) {
        setTickets(prev => prev.map(t => 
          t.id === editTicket.id ? { ...t, status: data.newStatus, stage: data.newStage, currentApprover: data.newApprover, formData: editFormData } : t
        ));
        setEditTicket(null);
        alert('成功重新送出申請！');
      } else {
        alert(data.error || '重新送出失敗');
      }
    } catch (error) {
  ...
  const fetchTickets = async (emailToFetch: string) => {
    ...
    setHasSearched(true);
    try {
      const res = await authFetch(`/api/tickets/my/${encodeURIComponent(emailToFetch.toLowerCase())}`);
      const data = await res.json();
    ...
    <div className="w-full">
              ...
              {expandedTicketId === ticket.id && (
                <div className="border-t border-slate-100 bg-slate-50 p-6 animate-slide-up">
                  ...
                  {ticket.status === 'Rejected' && (
                     <div className="mt-6 text-center">
                       <button 
                         onClick={(e) => { e.stopPropagation(); handleEditClick(ticket); }}
                         className="bg-rose-600 hover:bg-rose-500 text-white px-6 py-2 rounded-lg font-medium shadow-md transition-colors inline-flex items-center gap-2"
                        >
      ...
      {editTicket && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto animate-pop-in">
            <h3 className="text-xl font-bold flex items-center gap-2 mb-4 text-slate-800">
              <Edit3 className="w-6 h-6 text-emerald-500" />
              重新編輯單據：{editTicket.id}
            </h3>
            
            <div className="space-y-4 mb-6">
              <div>
                <label className="block text-slate-700 font-semibold mb-1">主旨</label>
                <input 
                  type="text" 
              ...
              {editTicket.amount !== undefined && editTicket.amount !== '' && (
                <div>
                  <label className="block text-slate-700 font-semibold mb-1">金額</label>
                  <input 
                    type="number" 
                    value={editFormData.amount || editTicket.amount || ''} 
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                className="flex-1 px-4 py-3 bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-500/30 font-semibold rounded-xl transition-all text-white disabled:opacity-50"
              >
                {isResubmitting ? '處理中...' : '確認重新送出'}
              </button>
            </div>
          </div>
        </div>
